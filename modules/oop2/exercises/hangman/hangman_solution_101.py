...
class HumanPlayer(Player):
    def turn(self, state):
        print(state)
        guess = input(f"{self.name}, guess a letter or type a word to solve the"
                      " puzzle: ")
        return guess
...
class ComputerPlayer(Player):
    ...
    def turn(self, state):
        matching = [word for word in self.vocab
                    if (state.match(word)
                        and len(set(word) & state.bad_guesses) == 0)]
        if len(matching) == 1:
            return matching[0]
        if len(matching) > 0:
            word = choice(matching)
            unguessed = set(word) & state.unguessed
            return choice(list(unguessed))
