...
class HumanPlayer(Player):
    ...
    def turn(self, state):
        ...
        """
        print(state)
        inpt = input(f"{self.name}, guess a letter or type a word to solve the"
                     " puzzle: ")
        return inpt
    
class ComputerPlayer(Player):
    """A computerized hangman player.
    ...
    def turn(self, state):
        ...
        Returns:
            str: the player's guess (a letter or a word).
        """
        possible = [w for w in self.vocab
                    if state.match(w) and not (set(w) & state.bad_guesses)]
        if len(possible) == 1:
            return possible[0]
        if possible:
            word = choice(possible)
            possible_guesses = state.unguessed & set(word)
        else:
            possible_guesses = state.unguessed
        return choice(list(possible_guesses))
