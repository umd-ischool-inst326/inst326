...
class HumanPlayer(Player):
    def turn(self, state):
        print(state)
        return input(f"{self.name}, type a letter or a word: ")
...
class ComputerPlayer(Player):
    ...
    def turn(self, state):
        vocab_list = [word for word in set(self.vocab) - state.guessed_words
                      if (state.match(word) and
                          len(state.bad_guesses & set(word)) == 0)]
        if len(vocab_list) == 1:
            return vocab_list[0]
        if state.blank_count < 3:
            return choice(vocab_list)
        word = choice(vocab_list)
        return choice(list(set(word) - state.guesses))
